...
/* This file is part of Arduino project for Winner Micro boards
  
     Original Arduino EEPROM.h by Paul Stoffregen
     https://github.com/PaulStoffregen/EEPROM

     Modified for w80x boards by Dmitry E. Dmitriev 
*/
...
#define EEPROM_DEFAULT_SIZE 0x0800          // 2048 bytes
...
#if defined(ARDUINO_STRING_SUPPORT)
#include <WString.h>    // need to include this so String is defined
#endif


#if defined( EEPROM_ADVANCED )
/***
    EERef class.
...
struct EEPROMClass{
    ...
    EEPROMClass(uint16_t size = EEPROM_DEFAULT_SIZE)                 { init(size); }

    uint32_t ee_start = 0; //0x80FF7FF;
    void init(uint16_t size)  
       {
        ee_start = EEPROM_END_ADDR - size;
       }
    
    
    uint8_t eeprom_read_byte( int idx ) {
        uint8_t buf;
        HAL_FLASH_Read(ee_start + idx, &buf, sizeof(buf));
        return buf;
    }
    
    uint8_t eeprom_write_byte( int idx, uint8_t val ) {
        uint8_t buf = val;
        if (HAL_OK == HAL_FLASH_Write(ee_start + idx, &buf, sizeof(buf)))  return 1;
        return 0;
    }

    uint8_t eeprom_read_block( int idx , uint8_t* ptr, uint16_t b_size) {
        if (HAL_OK == HAL_FLASH_Read(ee_start + idx, ptr, b_size)) return 1;
        return 0;
    }

    uint8_t eeprom_write_block( int idx , uint8_t* ptr, uint16_t b_size) {
        if (HAL_OK == HAL_FLASH_Write(ee_start + idx, ptr, b_size))  return 1;
        return 0;
    }
    
    
    //Basic user access methods.
    //int operator[]( const int idx )      { return read(idx); }
    uint8_t read( int idx )              { return eeprom_read_byte( idx ); }
    uint8_t write( int idx, uint8_t val )   { return eeprom_write_byte( idx, val ); }
    uint8_t update( int idx, uint8_t val )  { if (val != eeprom_read_byte( idx )) 
                                               return eeprom_write_byte( idx, val ); 
                                              else return 1; }

    //STL and C++11 iteration capability.
    //EEPtr begin()                        { return 0x00; }
    //EEPtr end()                          { return length(); } //Standards requires this to be the item after the last valid entry. The returned pointer is invalid.
    uint16_t length()                    { return EEPROM_END_ADDR - ee_start; }
    ...
    template< typename T > T &get( int idx, T &t ){
        ...
        eeprom_read_block(idx, ptr, sizeof(T));
        return t;
    }
    ...
    template< typename T > const T &put( int idx, const T &t ){
        ...
        eeprom_write_block(idx, ptr, sizeof(T));
